                  ...
                  int64_t ts, int64_t direction, int64_t operation)
{
  ...
  if (neighbor >= 0) {
    ...
    // TODO: Make sure there is no race condition here and that e->neighbor will be up to date.
...
stinger_remove_vertex (struct stinger *G, int64_t vtx_id)
{
  ...
  /* JP: This loop should no longer be necessary now that in edges will be capture in above loop
